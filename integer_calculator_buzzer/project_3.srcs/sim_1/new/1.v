...
module test();
    ...
    localparam  TIME_20US   =   20          ;//每个数码管刷新时间，默认20us；
    ...
    initial begin
        ...
        rst_n = 1;
        #(20*CYCLE);
        key_task(0,1);//按下第0行第1列按键；输入1
        key_task(1,2);//按下第1行第2列按键；输入6
        key_task(0,3);//按下第0行第3列按键，输入加号
        key_task(0,1);//按下第0行第1列按键；输入1
        key_task(2,2);//按下第2行第2列按键；输入9
        key_task(3,1);//按下第3行第1列按键，输入0
        key_task(3,0);//按下第3行第0列按键，输入等号
        key_task(3,3);//按下第3行第3列按键，切换模式
        key_task(0,3);//按下第0行第3列按键，开根号运算
        key_task(1,3);//按下第1行第3列按键，乘方运算
        key_task(3,0);//按下第3行第0列按键，输入等号
        key_task(0,3);//按下第0行第3列按键，开根号运算
        key_task(3,0);//按下第3行第0列按键，输入等号
        #(2000*CYCLE);
